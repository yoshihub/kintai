...
<div class="attendance-list-outer">
    <div class="attendance-title-bar">
        <span class="bar"></span>
        <span class="attendance-title">勤怠一覧</span>
    </div>
    <div class="attendance-header-row">
        <a href="{{ route('attendance.list', ['date' => $currentDate->copy()->subMonth()->format('Y-m')]) }}" class="month-btn">← 前月</a>
        <div class="attendance-month-label"><span class="calendar-icon">📅</span>{{ $currentDate->format('Y/m') }}</div>
        <a href="{{ route('attendance.list', ['date' => $currentDate->copy()->addMonth()->format('Y-m')]) }}" class="month-btn">翌月 →</a>
    </div>
        ...
        <table class="attendance-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($data['id'])
                        <a href="{{ route('attendance.detail', ['id' => $data['id']]) }}" class="detail-link">詳細</a>
                        @endif
                    </td>
                </tr>
                @endforeach
